...
class Game:
    ...
    def __init__(self, level = 1, respawn = False, check_point = None, death_count = 0, time_count = 0):
        ...
        # Game Board
        self.game_board = GameBoard(death_count = death_count, time_count = time_count)
    ...
    def mouseResponse(self, position: tuple, click):
        ...
        """
        self.status = self.menu_button.mouseInteraction(position, self.status, click)
        
        # Different response according to the screen mode
        if self.currentScreen == GAMEMENU:
            self.status = self.menuScreen.mouseInteraction(position, self.status, click)
        elif self.currentScreen == INSTRUCTIONSCREEN:
            self.status = self.instruction_screen.mouseInteraction(position, self.status, click)
    ...
    def readStatus(self):
        ...
        for stat in self.status:
            ...
            # Turn the screen mode to death screen
                ...
                self.time_count = (pygame.time.get_ticks() - self.game_board.start_time)//1000 + self.game_board.time_count
                ...
                self.time_count = (pygame.time.get_ticks() - self.game_board.start_time)//1000 + self.game_board.time_count
                self.win_text.set_time_death(self.time_count, self.game_board.death_count)
    ...
    def logic(self):
        ...
        if self.currentScreen == GAME:
            ...
            self.player.update(self.map.tileGroup, self.map.dead_zone, self.status,self.enemy_sprite_group, self.trap_group, self.map.disappear_tiles, self.map.vertices[0:2])
            self.status.append(self.player.checkdeath())
    ...
    def play(self) -> tuple[bool, bool, tuple, int] :
        ...
        while not self.done:
            ...
            # 60 ticks per second
            self.clock.tick(20)
        return self.restart, self.respawn_checkpoint, self.check_point, self.game_board.death_count,self.time_count, self.record
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, tiles: list, dead_zone: list, status: str, enemy_sprite_group: pygame.sprite.Group, trap_group: TrapGroup, disappear_tiles: list, x_boundary: list) -> bool:
        ...
        self.enemy_interaction(enemy_sprite_group, trap_group)
...
class MenuButton(Button):
    ...
    def __init__(self):
        super().__init__(40,40,(SIZE[0]- MENU_BUTTON_PADDING - 40, MENU_BUTTON_PADDING),"images/settingButton.png", SCREENTOGAMEMENU)
...
class GameMenuScreen():
    ...
    def mouseInteraction(self,position: tuple, status: list, click) -> list:
        """
...
class GameBoard():
    ...
    def __init__(self, death_count=0, time_count = 0):
        self.time = 0
    ...
    def draw(self, screen: pygame.Surface, cam_pos: pygame.math.Vector2):
        ...
        time = self.small_font.render(str((pygame.time.get_ticks() - self.start_time)//1000 + self.time_count), True, BLACK)
...
class WinText():
    ...
    def set_time_death(self, time, death):
        small_font = pygame.font.Font("freesansbold.ttf",40)
        self.time_txt = small_font.render("Time used: "+str(time) + "  Death count: "+ str(death), True, DARKBLUE)
        self.time_pos = (182+ (self.txt.get_size()[0] - self.time_txt.get_size()[0])/2, 130 + self.txt.get_size()[1])
        
        
    def draw(self,screen: pygame.Surface):
        screen.blit(self.txt, self.txt_pos)
        screen.blit(self.time_txt, self.time_pos)