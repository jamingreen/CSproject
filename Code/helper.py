...
class Button(pygame.sprite.Sprite, ABC):
    ...
    def __init__(self,width: int, height: int, position: tuple, imageName: str, status_code):
    ...
    def mouseInteraction(self,position: tuple, status: list, click):
        if click and self.rect.collidepoint(position) and not self.clicked:
            status.extend([self.status_code])
            self.clicked = True
        elif not click:
            self.clicked = False
        return status
...
class WordButton(ABC):
    ...
    def __init__(self,width: int, height: int, position: tuple, color: tuple, textColor: tuple, txt: str, status_code):
        super().__init__()
        ...
        self.clicked = False
        self.covered = False
        self.covered_color = (color[0] * 0.6, color[1] * 0.6, color[2] * 0.6)

    def mouseInteraction(self,position: tuple, status: list, click):
        self.covered = self.rect.collidepoint(position)
        if click and self.covered and not self.clicked:
            status.extend([self.status_code])
            self.clicked = True
        elif not click:
            self.clicked = False
    ...
    def draw(self, screen: pygame.Surface):
        if self.covered:
            pygame.draw.rect(screen, self.covered_color, self.rect, 0 )
        else:
            pygame.draw.rect(screen, self.color, self.rect, 0)
        screen.blit(self.txt, self.txt_pos)

class SettingButton(WordButton):

    def __init__(self, width: int, height: int, position: tuple):
        super().__init__(width, height,position,WHITE, BLACK,"Menu", SCREENTOSETTING)
...
class CloseButton(Button):
    ...
    def __init__(self,width: int, height: int, position: tuple, statusCode: str):
        super().__init__(width, height, position, os.path.join("images","closeButton.png"), statusCode)

class QuitButton(WordButton):

    def __init__(self, position: tuple):
        super().__init__(140,40, position, (51, 51, 204), WHITE, "Exit Game", CLOSEGAME)
    
class InstructionButton(WordButton):

    def __init__(self, position: tuple):
        super().__init__(140,40, position, (51, 51, 204), WHITE, "Instructions", SCREENTOINSTRUCTION)
    
class RankScreenButton(WordButton):

    def __init__(self, position: tuple):
        super().__init__(140,40, position, (51, 51, 204), WHITE, "Rankings", SCREENTORANKING)

class QuitGameButton(WordButton):

    def __init__(self, position: tuple):
        super().__init__(140, 40, position, (51, 51, 204), WHITE, "Quit Game", EXITGAME) 
...
class InstructionScreen():
    ...
    def __init__(self, closeStatusCode: str):
        ...
        # for generation class
        font = pygame.font.Font("freesansbold.ttf", FONTSIZE)
        
        # Generate surface object for the instuctions
        start_y = 108
        for instruction in self.instructions:
            txt = font.render(instruction, True, BLACK)
            fontSize = txt.get_size()
            
            # It is to center the text
            txt_pos = (76 + (648 - fontSize[0])/2, start_y + INSTRUCTION_MENU_PADDING)
            
            # Add the padding for the coordinate of the next text
...
class RestartButton(WordButton):
    ...
    def __init__(self, position: tuple):
        super().__init__(140,40, position, (51, 51, 204), WHITE, "Restart", RESTARTGAME)
...
class Spike:
    def __init__(self, position: tuple, base: int, height: int):
        # Position of the square the spike is int
    ...
    # The spike will made of 19 rectangle stacking on top of each other, mimicing a isosceles triangle. (The 20th has width of 0, so it will be ignore)
    def createRect(self):
        # The decrease in base for every rectangle on top
        base_change = self.base / 40
        height_change = self.height / 20
        ...
        # The coordinate of the first rectangle in the first iteration
        left = self.x + base_change
        top = self.y + self.height-height_change
        for _ in range(19):
            rect = pygame.Rect(left, top, base_length, height_change)
...
class SpikeUp(Spike, ActivateObjects):
    ...
    def __init__(self, position: tuple, base: int, height: int, zone: list, up: int, hori_dir = 0): #zone = (leftx, rightx, width, height)
        Spike.__init__(self, (position[0], position[1]), base, height)
        ActivateObjects.__init__(self,zone)
        self.tar_y = position[1] - up*BLOCKSIZE[1]
        
        # Number of block upward it should moves
        self.vert_dir = up
        # Number of block to the right it should moves
        self.hori_dir = hori_dir
        
        self.tar_x = position[0] + hori_dir * BLOCKSIZE[0]

    # Return if the player collides with the spike
    ...
    # Move the spike if it is activated and it has not reached its final position
    def logic(self):
        if self.activate:
            
            # Move up
            if self.y > self.tar_y and self.vert_dir > 0:
                self.y -= UP_SPEED
                for rect in self.rect_group:
                    rect.y -= UP_SPEED
            
            # Move down
            elif self.y < self.tar_y and self.vert_dir < 0:
                self.y += UP_SPEED
                for rect in self.rect_group:
                    rect.y += UP_SPEED
                
            # Move right
            if self.x < self.tar_x and self.hori_dir > 0:
                self.x += HORI_SPEED
                for rect in self.rect_group:
                    rect.x += HORI_SPEED
                
            # Move left
            elif self.x > self.tar_x and self.hori_dir < 0:
                self.x -= HORI_SPEED
                for rect in self.rect_group:
                    rect.x -= HORI_SPEED
